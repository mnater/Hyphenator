...
var  files = readFile("tmp/patternfiles.txt"), fileList = {},
fileName, i, name, path;
	//'Hyphenator_debug.js': readFile("dist/Hyphenator_debug.js")

files = files.split(':');
for (i=0; i<files.length; i++) {
	name = files[i].replace(/src\/patterns\//, '');
	path = files[i];
	fileList[name] = readFile(path);
}

function run (filename, src) {
	var options = {white: true, browser: true, devel: true, onevar: true, undef: false, nomen: true, eqeqeq: false, bitwise: false, regexp: true, newcap: true, immed: true, sub: true};
	JSLINT(src, options);
